/****************************************************************************
Copyright (c) 2012,重庆山外山科技有限公司技术中心
All rights reserved.

文件名称：	crc.cpp
摘     要 ：	CRC类实现

当前版本：    V1.0
作   者：	lp
修改内容：	创建该文件并实现该类的构造函数
完成日期：	2012-03-30
******************************************************************************/

#include "crc.h"
#include <string.h>
#include <stdlib.h>

/**********************************************************************************
函数名：         CRC()
功能描述：       CRC类的构造函数，在此初始化一些私有变量
输入参数：       base-----数据起始地址
                len-----数据长度
输出参数：       无
返回值：        无
***********************************************************************************/
CRC::CRC(unsigned char *base , unsigned short len)
{
    m_frame = base;
    m_length = len;

    if(m_frame)
        m_canDoCrc = true;
    else
        m_canDoCrc = false;
}

/**********************************************************************************
函数名：         sum()
功能描述：       对传入的数据求crc校验和
输入参数：       无
输出参数：       无
返回值：        crc校验和
***********************************************************************************/
unsigned short CRC::Sum()
{
    unsigned char CRCHi = 0xFF;
    unsigned char CRCLo = 0xFF;
    unsigned short iIndex;
    unsigned int i = 0;

    if(m_canDoCrc)
    {
        while ( m_length-- )
        {
            iIndex = CRCLo ^ ( m_frame[i++] ) ;
            CRCLo = ( unsigned char ) ( CRCHi ^ s_aucCRCHi[iIndex] );
            CRCHi = s_aucCRCLo[iIndex];
        }
        return ( unsigned short ) ( CRCHi << 8 | CRCLo );// CRC校验返回值
    }
    else
    {
        return -1;
    }
}

/**********************************************************************************
函数名：         ～CRC()
功能描述：       CRC类的析构函数，释放一些私有变量对象
输入参数：       无
输出参数：       无
返回值：        无
***********************************************************************************/
CRC::~CRC( )
{
    if(m_frame)
        m_frame = NULL;
}

/*********************************************************************
*  静态数组定义
*********************************************************************/
const unsigned char CRC::s_aucCRCHi[] =
{   
};

const unsigned char CRC::s_aucCRCLo[] =
{   
};

