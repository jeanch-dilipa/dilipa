/****************************************************************************
Copyright (c) 2022 dilipa All rights reserved.

fileName:   crc.cpp
    info:	the cyclic redundancy check
******************************************************************************/

#include <string.h>
#include <stdlib.h>

#include "crc.h"

/****************************************************************************
接口描述: CRC校验构造函数
输   入: base---数据起始地址, len---数据长度
输   出: /
******************************************************************************/
CRC::CRC(unsigned char *base , unsigned short len)
{
    m_frame = base;
    m_length = len;

    if(m_frame)
    {
        m_canDoCrc = true;
    }
    else
    {
        m_canDoCrc = false;
    }
}

/****************************************************************************
接口描述: CRC校验和
输   入: /
输   出: /
******************************************************************************/
unsigned short CRC::Sum()
{
    unsigned char CRCHi = 0xFF;
    unsigned char CRCLo = 0xFF;
    unsigned short iIndex;
    unsigned int i = 0;

    if(m_canDoCrc)
    {
        while (m_length--)
        {
            iIndex = CRCLo ^ (m_frame[i++]) ;
            CRCLo = static_cast<unsigned char>(CRCHi ^ s_aucCRCHi[iIndex]);
            CRCHi = s_aucCRCLo[iIndex];
        }

        return static_cast<unsigned short>(CRCHi << 8 | CRCLo);                           // CRC校验返回值
    }
    else
    {
        return -1;
    }
}

/****************************************************************************
接口描述: CRC校验析构函数
输   入: /
输   出: /
******************************************************************************/
CRC::~CRC( )
{
    if(nullptr != m_frame)
    {
        m_frame = nullptr;
    }
}

/*****************************************************************************
*  静态数组定义
******************************************************************************/
const unsigned char CRC::s_aucCRCHi[] =
{   
};

const unsigned char CRC::s_aucCRCLo[] =
{   
};

