...
Window {
    ...
    FileDialog{
        ...
        nameFilters: [qsTr("升级文件") + "(SWS-6000-Revolution; *.db; *.qm; *.bin; compatibility; language.ini)"];
        onAccepted: {
    ...
    function resolveFileUrls(fileUrls)
    {
        for(var i = 0; i < fileUrls.length; i++)
        {
            ...
            else if(fileName.match(/^.*\.qm$/))
            {   //语言
                addUpdateFileToItem("Language", fileName, fileTotalName);
            }
            else if(fileName === "language.ini")
            {   //语言配置文件
                addUpdateFileToItem("LanguageIni", fileName, fileTotalName);
            }
            ...
            else if(fileName === "compatibility")
            {   //软硬件兼容表
    ...
    function addUpdateFileToItem(fileType, fileName, fileTotalName){
        ...
        if(children.length === 0 && outputFileField.text === "")
        {   //当前还没有添加文件且输出文件名未设置时，默认设置到第一个添加的位置
            outputFileField.text = fileTotalName.slice(0, fileTotalName.lastIndexOf("/") + 1) + "untitledPacket.upk";
        }
        ...
        {   //没有找到，创建一个            
            ...
            if(fileType === "Comp" || fileType === "LanguageIni")
            {
                obj.showTxtFile.connect(updatePacking.slotShowTxtFile);
            }
        }
    ...
    RowLayout{
        ...
        NormalButton {
            ...
            onClicked: {
                //查找有没有添加了bin文件但没有添加软硬件兼容表的情况，进行提示
                var children = srollFlow.children;
                var hasModule = false;
                var hasComp = false;
                for(var i = 0; i < children.length; i++) //遍历所有升级项
                {
                    if(children[i].type === "Comp")
                    {
                        hasComp = true;
                    }
                    else if(children[i].isModuleBoard)
                    {
                        hasModule = true;
                    }
                }
                if(hasModule && !hasComp)
                {
                    noticeDialog.message = qsTr("未添加“软硬件兼容表（compatibility）”文件，添加的bin文件可能无法升级，确定要继续打包吗？");
    ...
    function packingPrepare()
    {
        ...
        packingBtn.startPacking(outputFileField.text, packetVerField.text, fileTotalArray, assignVerList, minNoRebootVerField.text);
    }

    FileDialog{ //输出升级包位置选择文件对话框
        id: outputFileDialog;
        fileMode: FileDialog.SaveFile;
        nameFilters: [qsTr("升级包文件") + " (*.upk)"];
