...
void UpdateFilePacking::slotShowTxtFile(QString filePath)
{
    if(m_showTxtFileList.contains(filePath))
    {
        return;
    }
    ...
    {
        m_showTxtFileList.append(filePath);

        QProcess *process = new QProcess(this);
        connect(process, (void (QProcess:: *)(int))&QProcess::finished, this, [this](int /*exitCode*/){
            QProcess *senderProcess = (QProcess *)sender();
            if(senderProcess)
            {
                QStringList arguments = senderProcess->arguments();
                if(!arguments.isEmpty())
                {
                    m_showTxtFileList.removeAll(arguments.at(0));
                }
